...
def playaudio(msg):
    ...
    msg_wait = bot.send_message(msg.chat.id, f"🔍 Buscando: {query}...")
    ...
    try:
        ...
        bot.delete_message(msg.chat.id, msg_wait.message_id)
        bot.send_message(msg.chat.id, f"⬇️ Descargando: {title}")
        
        with open(filename, 'rb') as audio:
            bot.send_audio(msg.chat.id, audio=audio, title=title, caption=title)
        os.remove(filename)
        
    except Exception as e:
        bot.send_message(msg.chat.id, f"❌ Error al descargar\nIntenta con el link de YouTube")
        print("Error:", e)
